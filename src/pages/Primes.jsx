...
// ISO week dates (Monday-based) for week/year
function primeWeekDates(wn, year) {
...
// Return sorted day-numbers of the month where empId has an approved leave
function empLeaveDaysInMonth(leaveRequests, empId, year, month) {
  const days = new Set();
  (leaveRequests || []).filter(r => r.employeeId === empId && r.status === 'approved').forEach(req => {
    (req.weeks || []).forEach(w => {
      const wd = primeWeekDates(w.week, w.year);
      (w.days || []).forEach(di => {
        const date = wd[di];
        if (date && date.getMonth() === month && date.getFullYear() === year) days.add(date.getDate());
      });
    });
  });
...
// Aggregate per-day lateness (minutes) for empId in a month, scanning ALL stores' schedules.
// Returns { total, entries:[{date, min}] }
function empLatenessInMonth(schedules, empId, year, month) {
  const entries = [];
  let total = 0;
  const WORK = ['work','communication','meeting','school'];
  Object.entries(schedules || {}).forEach(([key, sched]) => {
    // key format: storeId_YEAR_Wxx
    const m = key.match(/_(\d{4})_W(\d+)$/);
    if (!m) return;
    const kYear = parseInt(m[1]), wn = parseInt(m[2]);
    if (kYear !== year && kYear !== year + 1 && kYear !== year - 1) return; // week may span year boundary
    const wd = primeWeekDates(wn, kYear);
    for (let di = 0; di < 7; di++) {
      const sh = sched[`${empId}_${di}`];
      if (!sh || !(sh.latenessMin > 0)) continue;
      const date = wd[di];
      if (date && date.getMonth() === month && date.getFullYear() === year) {
        entries.push({ date: date.getDate(), min: sh.latenessMin });
        total += sh.latenessMin;
      }
    }
  });
...
// Nombre total de jours ouvrés du mois (lun-sam, hors fériés)
function totalWorkDays(year, month, holidays) {
...
function VendeurCard({ emp, data, onChange, storeBonusPool, overtimeToPay = 0, leaveDays = [], monthLabel = '', lateness = { total: 0, entries: [] } }) {
  const v = data || {};
  ...
  const autoOvertimeRaw = Number(overtimeToPay) || 0;
  const autoOvertime = v.ignoreAutoOvertime ? 0 : autoOvertimeRaw;
    ...
    <div style={{ background: 'var(--card)', border: `1.5px solid ${open ? 'var(--teal-mid)' : 'var(--border)'}`, borderRadius: 16, marginBottom: 14, overflow: 'hidden', boxShadow: open ? '0 6px 24px rgba(0,201,177,.13)' : '0 1px 6px rgba(0,0,0,.05)' }}>
      ...
      {open && (
        <div style={{ padding: '6px 18px 20px' }}>
          ...
          <div style={{ background: 'var(--card)', border: '1.5px solid #F5B764', borderRadius: 14, padding: '14px 16px' }}>
            {autoOvertimeRaw > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 12, paddingBottom: 12, borderBottom: '1px dashed var(--border)' }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: v.ignoreAutoOvertime ? 'var(--muted)' : '#B05A00', textDecoration: v.ignoreAutoOvertime ? 'line-through' : 'none' }}>Heures supp. auto (validées) : {fmtHrs(autoOvertimeRaw)} h</div>
                  <div style={{ fontSize: 12, color: 'var(--muted)' }}>Calculées automatiquement depuis les heures validées</div>
                </div>
                <label style={{ display: 'flex', alignItems: 'center', gap: 7, cursor: 'pointer', flexShrink: 0 }}>
                  <input type="checkbox" checked={!!v.ignoreAutoOvertime} onChange={e => set('ignoreAutoOvertime', e.target.checked)} style={{ width: 18, height: 18, cursor: 'pointer', accentColor: '#C8002B' }} />
                  <span style={{ fontSize: 13, fontWeight: 700, color: v.ignoreAutoOvertime ? '#C8002B' : 'var(--muted)' }}>Retirer</span>
                </label>
              </div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#B05A00' }}>Heures supp. manuelles (h)</div>
                <div style={{ fontSize: 12, color: 'var(--muted)' }}>Saisie manuelle · affichée à part, ne s'ajoute pas à la prime</div>
              </div>
            ...
            {totalOvertime > 0 && <div style={{ marginTop: 10, fontSize: 13.5, fontWeight: 700, color: '#B05A00', textAlign: 'right' }}>Total à payer : {fmtHrs(totalOvertime)} h</div>}
          </div>

          <SectionLabel>Retards & absences</SectionLabel>
          <div style={{ background: 'var(--card)', border: `1.5px solid ${v.hasLateness ? '#F5B5C0' : 'var(--border)'}`, borderRadius: 14, padding: '14px 16px' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: v.hasLateness ? 10 : 0 }}>
              <input type="checkbox" checked={!!v.hasLateness} onChange={e => set('hasLateness', e.target.checked)} style={{ width: 20, height: 20, cursor: 'pointer', accentColor: '#C8002B' }} />
              <span style={{ fontSize: 14, fontWeight: 700, color: v.hasLateness ? '#C8002B' : 'var(--text)' }}>⏰ A eu des retards ce mois</span>
            </label>
            {v.hasLateness && (
              <input className="inp" type="text" value={v.latenessNote ?? ''} onChange={e => set('latenessNote', e.target.value)} placeholder="Commentaire (dates, détails...)" style={{ fontSize: 14, padding: '9px 12px' }} />
            )}
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginTop: 12, marginBottom: v.hasAbsence ? 10 : 0 }}>
              <input type="checkbox" checked={!!v.hasAbsence} onChange={e => set('hasAbsence', e.target.checked)} style={{ width: 20, height: 20, cursor: 'pointer', accentColor: '#D97706' }} />
              <span style={{ fontSize: 14, fontWeight: 700, color: v.hasAbsence ? '#B05A00' : 'var(--text)' }}>🚫 A eu des absences ce mois</span>
            </label>
            {v.hasAbsence && (
              <input className="inp" type="text" value={v.absenceNote ?? ''} onChange={e => set('absenceNote', e.target.value)} placeholder="Commentaire (dates, détails...)" style={{ fontSize: 14, padding: '9px 12px' }} />
            )}
          </div>
          <div style={{ marginTop: 18, background: 'linear-gradient(135deg,var(--teal-light),#EEF6FF)', border: '1.5px solid var(--teal-mid)', borderRadius: 14, padding: '16px 18px' }}>
            ...
            {v.hasLateness && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, marginTop: 6, color: '#C8002B' }}><span>⏰ Retards ce mois</span><strong>Oui{v.latenessNote ? ` · ${v.latenessNote}` : ''}</strong></div>}
            {v.hasAbsence && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, marginTop: 4, color: '#B05A00' }}><span>🚫 Absences ce mois</span><strong>Oui{v.absenceNote ? ` · ${v.absenceNote}` : ''}</strong></div>}
            {leaveDays.length > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, fontSize: 14, marginTop: 4, color: '#7C3AED' }}><span>🌴 Congés {monthLabel}</span><strong style={{ textAlign: 'right' }}>{leaveDays.length} j · {leaveDays.join(', ')}</strong></div>}
            {lateness.total > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, fontSize: 14, marginTop: 4, color: '#C8002B' }}><span>⏰ Retards {monthLabel}</span><strong style={{ textAlign: 'right' }}>{lateness.total} min ({lateness.entries.map(e => `${e.date}: ${e.min}min`).join(', ')})</strong></div>}
...
export default function Primes() {
  const { stores, employees, getVisibleStoreIds, primes, savePrimeData, currentEmp, currentUser, isDirigeant, authRole, primeRequests, approvePrimeChangeRequest, rejectPrimeChangeRequest, getEmpOvertimeToPay, leaveRequests, schedules } = useApp();
  const now = new Date();
    ...
    <div className="anim-up">
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 28 }}>
        <button className="btn btn-primary btn-sm" onClick={() => {
          const sdata = visibleStores.map(s => ({ store: s, data: buildStorePrime(s, employees, primes[keyFor(s.id)] || {}, getEmpOvertimeToPay, year, month, leaveRequests, schedules) }));
        ...
        {isDirigeant && (
          <>
            <button className="btn btn-ghost btn-sm" onClick={() => {
              const sdata = stores.map(s => ({ store: s, data: buildStorePrime(s, employees, primes[keyFor(s.id)] || {}, getEmpOvertimeToPay, year, month, leaveRequests, schedules) }));
      ...
      {visibleStores.map(store => {
        ...
        const totalWD = totalWorkDays(year, month, holidays);
        const elapsedWD = Math.max(0, totalWD - workDaysLeft);
        const dailyPace = elapsedWD > 0 ? storeMargin / elapsedWD : 0;
        const projection = elapsedWD > 0 ? dailyPace * totalWD : null;
        const projP1 = palier1 > 0 && projection != null ? projection >= palier1 : false;
        const projP2 = palier2 > 0 && projection != null ? projection >= palier2 : false;
        const projLabel = projection == null ? '' : (projP2 && !palier2Invalid ? 'Palier 2 (3 %)' : (projP1 ? 'Palier 1 (1,5 %)' : 'Aucun palier'));
        const projPool = projection == null ? 0 : (projP2 && !palier2Invalid ? projection * 0.03 : (projP1 ? projection * 0.015 : 0));
        const projColor = projection == null ? 'var(--muted)' : (projP2 && !palier2Invalid ? '#1A8A42' : (projP1 ? '#0D9488' : '#C8002B'));
        const projVsLastYear = lastYearMargin > 0 && projection != null ? ((projection - lastYearMargin) / lastYearMargin) * 100 : null;
        ...
        return (
          <div key={store.id} style={{ marginBottom: 38 }}>
            ...
            <div style={{ background: 'var(--card)', border: `2px solid ${store.color}40`, borderRadius: 18, padding: '20px', marginBottom: 18 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 14 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {lastYearMargin > 0 && (
                    <div style={{ background: '#EEF6FF', border: '1.5px solid var(--teal-mid)', borderRadius: 12, padding: '12px 14px' }}>
                      <div style={{ fontSize: 13, color: 'var(--muted)', fontWeight: 600, marginBottom: 4 }}>Pour égaler {MONTHS[month]} {year - 1} ({eur(lastYearMargin)})</div>
                      {storeMargin >= lastYearMargin ? (
                        <div style={{ fontSize: 15, fontWeight: 700, color: '#1A8A42' }}>✅ Objectif déjà atteint</div>
                      ) : (
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 10 }}>
                          <div>
                            <div style={{ fontSize: 12.5, color: 'var(--muted)' }}>Reste {eur(lastYearMargin - storeMargin)} sur {workDaysLeft} j</div>
                          </div>
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--teal-dark)' }}>{workDaysLeft > 0 ? eur((lastYearMargin - storeMargin) / workDaysLeft) : '—'}</div>
                            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--muted)' }}>par jour</div>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  {projection != null && (
                    <div style={{ background: projColor + '10', border: `1.5px solid ${projColor}55`, borderRadius: 12, padding: '13px 15px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 10, marginBottom: 6 }}>
                        <div style={{ fontSize: 13, color: 'var(--muted)', fontWeight: 700 }}>📈 Projection fin de mois (même rythme)</div>
                        <div style={{ fontSize: 21, fontWeight: 800, color: projColor }}>{eur(projection)}</div>
                      </div>
                      <div style={{ fontSize: 12.5, color: 'var(--muted)', marginBottom: 6 }}>
                        Rythme actuel : <strong>{eur(dailyPace)}/j</strong> sur {elapsedWD} j faits · {workDaysLeft} j restants ({totalWD} j au total)
                      </div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                        <span style={{ background: projColor + '18', color: projColor, border: `1px solid ${projColor}44`, borderRadius: 7, padding: '3px 10px', fontSize: 12.5, fontWeight: 700 }}>
                          {projLabel}{projPool > 0 ? ` · enveloppe ~${eur(projPool)}` : ''}
                        </span>
                        {projVsLastYear != null && (
                          <span style={{ background: projVsLastYear >= 0 ? '#E8FAF0' : '#FFF0F2', color: projVsLastYear >= 0 ? '#1A8A42' : '#C8002B', border: `1px solid ${projVsLastYear >= 0 ? '#A5D6A7' : '#F5B5C0'}`, borderRadius: 7, padding: '3px 10px', fontSize: 12.5, fontWeight: 700 }}>
                            {projVsLastYear >= 0 ? '▲' : '▼'} {projVsLastYear > 0 ? '+' : ''}{projVsLastYear.toFixed(1)} % vs N-1
                          </span>
                        )}
                      </div>
                    </div>
            ...
            {storeEmps.length === 0 ? (
              ...
              storeEmps.map(emp => (<VendeurCard key={emp.id} emp={emp} data={vendeurs[emp.id]} onChange={(id, vData) => updateVendeur(store.id, id, vData)} storeBonusPool={storeBonusPool} overtimeToPay={getEmpOvertimeToPay ? getEmpOvertimeToPay(emp.id, year, month) : 0} leaveDays={empLeaveDaysInMonth(leaveRequests, emp.id, year, month)} monthLabel={MONTHS[month]} lateness={empLatenessInMonth(schedules, emp.id, year, month)} />))
            )}
