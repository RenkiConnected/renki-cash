...
export default function ViewPlanning() {
  ...
  return (
    <div className="anim-up">
      ...
      {/* Mon récap de la semaine : où je travaille chaque jour */}
      {myEmp && viewMode==='week' && (()=>{
        const days=['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'];
        const cells=weekDates.slice(0,7).map((wd,di)=>{
          let where=null, sh=null;
          const homeSched=getSchedule(myEmp.storeId,currentWeek,currentYear);
          const hs=homeSched[`${myEmp.id}_${di}`];
          if(hs && WORK.includes(hs.type)){ where=stores.find(s=>s.id===myEmp.storeId); sh=hs; }
          else {
            for(const st of stores){
              if(st.id===myEmp.storeId) continue;
              const os=getSchedule(st.id,currentWeek,currentYear)[`${myEmp.id}_${di}`];
              if(os && WORK.includes(os.type)){ where=st; sh=os; break; }
            }
            if(!sh && hs) sh=hs;
          }
          return {di,wd,where,sh};
        });
        return (
          <div style={{background:'var(--card2)',border:'1.5px solid var(--border)',borderRadius:12,padding:'12px 14px',marginBottom:18}}>
            <div style={{fontSize:13,fontWeight:800,color:'var(--muted)',marginBottom:8}}>📍 Ma semaine en un coup d'œil</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:6}}>
              {cells.map(({di,wd,where,sh})=>{
                const st=sh?getShiftMeta(sh.type):null;
                const isWork=sh&&WORK.includes(sh.type);
                return (
                  <div key={di} style={{textAlign:'center',background:'#fff',border:`1.5px solid ${where?where.color+'55':'var(--border)'}`,borderRadius:9,padding:'7px 4px'}}>
                    <div style={{fontSize:11,fontWeight:700,color:'var(--muted)'}}>{days[di]} {wd.date.getDate()}</div>
                    {isWork&&where?(
                      <>
                        <div style={{fontSize:12,fontWeight:800,color:where.color,marginTop:3,lineHeight:1.1}}>{where.id===myEmp.storeId?where.name:`✈ ${where.name}`}</div>
                        {sh.startTime&&<div style={{fontSize:10,color:'var(--muted)',marginTop:1}}>{sh.startTime}–{sh.endTime}</div>}
                      </>
                    ):(
                      <div style={{fontSize:11,fontWeight:600,color:'var(--muted)',marginTop:3}}>{st?st.label:'—'}</div>
                    )}
                  </div>
                );
              })}
