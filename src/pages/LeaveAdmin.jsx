                ...
                return(
                  <div key={r.id} style={{background:r.status==='approved'?'#E8FAF0':r.status==='refused'?'#FFF0F2':'#fff',border:`1.5px solid ${r.status==='approved'?'var(--teal-mid)':r.status==='refused'?'#FFCCD4':'var(--border)'}`,borderRadius:14,padding:'18px 22px',boxShadow:'var(--shadow)'}}>
                    ...
                    <div style={{padding:'12px 16px',background:'rgba(0,0,0,.03)',borderRadius:10,marginBottom:12}}>
                      ...
                      {r.excType&&(
                        <div style={{marginBottom:8,padding:'8px 12px',borderRadius:9,fontWeight:700,fontSize:14,display:'inline-block',
                          background:r.excType==='school'?'#FEF08A':r.excType==='exam'?'#CCFBF1':'#FEF9C3',
                          color:r.excType==='school'?'#B45309':r.excType==='exam'?'#0D9488':'#CA8A04'}}>
                          {r.excType==='school'?'📚 École (journée 7h auto)':r.excType==='exam'?'📝 Examen (journée 7h auto)':'☕ Repos'}
                          <span style={{fontWeight:500,fontSize:12,marginLeft:6,color:'var(--muted)'}}>· placé automatiquement à l'approbation</span>
                        </div>
                      )}
