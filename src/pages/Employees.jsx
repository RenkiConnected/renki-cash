    ...
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display:'grid', gap:16 }}>
          ...
          <div style={{ background: form.endDate ? '#FFF0F2' : 'var(--card2)', border:`1.5px solid ${form.endDate ? '#F5B5C0' : 'var(--border)'}`, borderRadius:14, padding:'14px 16px' }}>
            <div style={{ fontSize:14, fontWeight:800, marginBottom:8 }}>🚪 Fin de présence (fin de contrat)</div>
            <label className="lbl">Ne plus afficher dans les plannings à partir du</label>
            <input className="inp" type="date" value={form.endDate || ''} onChange={e=>set('endDate', e.target.value || null)} />
            <div style={{ fontSize:11.5, color:'var(--muted)', marginTop:6 }}>
              {form.endDate
                ? `La personne reste modifiable jusqu'au ${new Date(form.endDate+'T12:00').toLocaleDateString('fr-FR')} inclus, disparaît des plannings après, mais reste visible sur les anciens plannings.`
                : 'Laissez vide si la personne reste en poste. Sinon, elle disparaîtra des plannings postérieurs à cette date tout en restant sur les anciens.'}
            </div>
          </div>
          <div>
