...
export default function LeaveRequestPage() {
  ...
  const [cExcType, setCExcType] = useState('');
          ...
          {ctab==='exceptional'&&(
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:22}}>
              <div className="card" style={{padding:'28px'}}>
                ...
                <p style={{color:'var(--muted)',fontSize:14,marginBottom:22}}>Signalez à votre manager un jour d'École, d'Examen ou de Repos. École et Examen comptent automatiquement comme une journée de 7h de travail. Tout autre motif (décès, imprévu…) est traité comme un repos.</p>
                ...
                <div style={{marginBottom:16}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
                    {[
                      {id:'school',label:'École',icon:'📚',color:'#B45309',bg:'#FEF08A',sub:'7h auto'},
                      {id:'exam',label:'Examen',icon:'📝',color:'#0D9488',bg:'#CCFBF1',sub:'7h auto'},
                      {id:'rest',label:'Repos',icon:'☕',color:'#CA8A04',bg:'#FEF9C3',sub:'motif divers'},
                    ].map(opt=>(
                      <button key={opt.id} type="button" onClick={()=>setCExcType(opt.id)} style={{
                        padding:'14px 8px',borderRadius:12,cursor:'pointer',textAlign:'center',
                        border:`2px solid ${cExcType===opt.id?opt.color:opt.color+'30'}`,
                        background:cExcType===opt.id?opt.bg:'#fff'}}>
                        <div style={{fontSize:22}}>{opt.icon}</div>
                        <div style={{fontSize:13.5,fontWeight:700,color:cExcType===opt.id?opt.color:'var(--text)',marginTop:4}}>{opt.label}</div>
                        <div style={{fontSize:10.5,color:'var(--muted)',marginTop:1}}>{opt.sub}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div style={{marginBottom:22}}>
                  <label className="lbl">Motif / précision (facultatif)</label>
                  <textarea className="inp" rows={3} value={cNote} onChange={e=>setCNote(e.target.value)} placeholder="Ex: examen du permis, journée de formation, raison personnelle..."/>
                </div>

                <button className="btn btn-primary" style={{width:'100%',justifyContent:'center',padding:'14px',fontSize:15}}
                  disabled={!cDate||!cExcType||cSubmitting}
                  onClick={async()=>{
                    if(!cDate||!cExcType) return;
                    setCSubmitting(true);
                    // École/Examen = journée 7h auto ; Repos (et tout autre motif) = repos
                    const isWork = cExcType==='school'||cExcType==='exam';
                    await submitConstraintRequest({type:'exceptional',date:cDate,excType:cExcType,autoType:cExcType,autoHours:isWork?7:null,note:cNote});
                    setCSubmitting(false); setCSuccess(true); setCDate(''); setCExcType(''); setCNote('');
                    setTimeout(()=>setCSuccess(false),4000);
                  }}>
                  {cSubmitting?'⏳ Envoi...':'✓ Envoyer la demande'}
                </button>
              </div>
                ...
                {myConstraints.filter(r=>r.type==='exceptional').length===0?(
                  ...
                  myConstraints.filter(r=>r.type==='exceptional').map(r=>(
                    <div key={r.id} style={{background:r.status==='approved'?'#E8FAF0':r.status==='refused'?'#FFF0F2':'#FFF7E0',border:`1.5px solid ${r.status==='approved'?'var(--teal-mid)':r.status==='refused'?'#FFCCD4':'#F5D06A'}`,borderRadius:12,padding:'14px 16px',marginBottom:10}}>
                      ...
                      <div style={{padding:'10px 14px',background:'rgba(255,255,255,.7)',borderRadius:9,fontSize:14,fontWeight:700,color:'var(--text)'}}>
                        {r.excType==='school'?'📚 École (journée 7h)':r.excType==='exam'?'📝 Examen (journée 7h)':'☕ Repos'}
                      </div>
