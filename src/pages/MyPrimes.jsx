...
// Remaining worked days (Mon–Sat) in a month from today, excluding holiday dates (YYYY-MM-DD)
...
export default function MyPrimes() {
  ...
  if (p2>0 && storeMargin>=p2 && !p2Invalid) { pool=storeMargin*0.03; palierLabel='Palier 2 atteint (3 %)'; }
  else if (p1>0 && storeMargin>=p1) { pool=storeMargin*0.015; palierLabel=(p2Invalid&&p2>0&&storeMargin>=p2)?'Palier 2 invalidé → Palier 1 (1,5 %)':'Palier 1 atteint (1,5 %)'; }
  const sharePct = Number(v.storeBonusPct)||0;
  const workDaysLeft = remainingWorkDays(year, month, sd.holidays || []);
  ...
  // Team members of this store (read-only view for the vendeur)
  ...
  // Team members of this store (read-only view for the vendeur)
  const teamEmps = employees.filter(e => e.storeId === resolvedStoreId && (e.role === 'vendeur' || e.role === 'manager'));
  const teamData = teamEmps.map(emp => {
    const ev = (sd.vendeurs && sd.vendeurs[emp.id]) || {};
    const eBase = vendeurBase(ev);
    const eShare = pool * (Number(ev.storeBonusPct)||0) / 100;
    return { emp, base:eBase, share:eShare, total:eBase+eShare, travel:Number(ev.travel)||0, pct:Number(ev.storeBonusPct)||0 };
  ...
  return (
    <div className="anim-up">
      ...
      {/* ── Paliers du magasin (lecture) ── */}
      <div className="card" style={{ padding:'22px', marginBottom:16 }}>
        <h2 className="section-title" style={{ marginBottom:6 }}>Paliers du magasin</h2>
        <p style={{ fontSize:13, color:'var(--muted)', marginBottom:14 }}>Marge cumulée : <strong>{eur(storeMargin)}</strong> · <span style={{ fontWeight:700, color: pool>0 ? (palierLabel.includes('invalidé')?'#E08A00':'#1A8A42') : 'var(--muted)' }}>{palierLabel}</span></p>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:12 }}>
          {[{n:1,p:p1,pct:'1,5 %'},{n:2,p:p2,pct:'3 %'}].map(pl=>{
            const reached = pl.p>0 && storeMargin>=pl.p;
            const rem = Math.max(0, pl.p - storeMargin);
            const invalidated = pl.n===2 && p2Invalid && reached;
            return (
              <div key={pl.n} style={{ background: reached&&!invalidated?'#E8FAF0':(invalidated?'#FFF7E0':'var(--card2)'), border:`1.5px solid ${reached&&!invalidated?'#A5D6A7':(invalidated?'#F5C96B':'var(--border)')}`, borderRadius:14, padding:'14px 16px' }}>
                ...
                {pl.p<=0 ? <div style={{ fontSize:13, color:'var(--muted)', fontStyle:'italic' }}>Non défini</div>
                  : invalidated ? <div style={{ fontSize:14, color:'#B05A00', fontWeight:700 }}>⚠️ Atteint mais invalidé (conditions non remplies)</div>
                  : reached ? <div style={{ fontSize:15, color:'#1A8A42', fontWeight:700 }}>✅ Atteint</div>
                  : <>
                      <div style={{ display:'flex', justifyContent:'space-between', fontSize:14, marginBottom:4 }}>
                        <span style={{ color:'var(--muted)' }}>Restant</span>
                        <strong style={{ color:store?.color||'var(--teal-dark)' }}>{eur(rem)}</strong>
                      </div>
                      <div style={{ display:'flex', justifyContent:'space-between', fontSize:14 }}>
                        <span style={{ color:'var(--muted)' }}>Par jour ({workDaysLeft} j restants)</span>
                        <strong style={{ color:'var(--teal-dark)' }}>{workDaysLeft>0?eur(rem/workDaysLeft)+'/j':'—'}</strong>
                      </div>
                    </>}
              </div>
            );
      ...
      {/* ── Chiffres de l'équipe (lecture seule) ── */}
      <div className="card" style={{ padding:'22px', marginBottom:16 }}>
        <h2 className="section-title" style={{ marginBottom:6 }}>Chiffres de l'équipe</h2>
        <p style={{ fontSize:13, color:'var(--muted)', marginBottom:14 }}>{store?.name||'Boutique'} · {MONTHS[month]} {year} · lecture seule</p>
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {teamData.map(({emp,base,share,total,travel,pct})=>{
            const isMe = emp.id===me.id;
            return (
              <div key={emp.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', background:isMe?'var(--teal-light)':'var(--card2)', border:`1.5px solid ${isMe?'var(--teal-mid)':'var(--border)'}`, borderRadius:12 }}>
                <div style={{ width:36, height:36, borderRadius:'50%', background:emp.color||'var(--teal)', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:15, flexShrink:0 }}>{emp.name[0]}</div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:700, fontSize:15 }}>{emp.name}{isMe&&<span style={{ fontSize:11, color:'var(--teal-dark)', marginLeft:6 }}>(moi)</span>}</div>
                  <div style={{ fontSize:12, color:'var(--muted)' }}>Ventes {eur(base)} · Part magasin {eur(share)} ({pct}%){travel>0?` · Frais ${eur(travel)}`:''}</div>
                </div>
                <div style={{ fontSize:18, fontWeight:800, color:'var(--teal-dark)', flexShrink:0 }}>{eur(total)}</div>
              </div>
            );
          })}
        </div>
