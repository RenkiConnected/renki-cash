...
import { exportToPDF, exportToNotion, exportAllStoresPlanningPDF } from './utils/pdfExport';
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const allPlanning = async (e) => {
      ...
      try { await exportAllStoresPlanningPDF({ stores, employees, schedules, weekDates: getWeekDatesForCurrentWeek(week), shiftTypes, currentWeek: week, currentYear, logoDataUrl, getContractH: (emp)=>effectiveContractHours(emp, week, currentYear) }); }
      catch (err) { alert('Erreur PDF toutes boutiques : ' + err.message); }
    };
    window.addEventListener('exportAllPlanning', allPlanning);
    return () => { window.removeEventListener('exportPDF', pdf); window.removeEventListener('exportNotion', notion); window.removeEventListener('exportAllPlanning', allPlanning); };
