...
import { SHIFT_TYPES as SEED_SHIFT_TYPES } from '../firebaseService';
...
export function AppProvider({ children }) {
  ...
  useEffect(()=>{
    ...
    const u3=listenShiftTypes((types)=>{
      // Merge Firestore types with code-defined seed types so newly added types
      // (e.g. 'exam') always appear even if the DB was seeded before they existed.
      const byId={};
      (types||[]).forEach(t=>{ byId[t.id]=t; });
      SEED_SHIFT_TYPES.forEach(t=>{ if(!byId[t.id]) byId[t.id]=t; });
      // Preserve seed order, then any extra custom types
      const ordered=[...SEED_SHIFT_TYPES.map(t=>byId[t.id]), ...Object.values(byId).filter(t=>!SEED_SHIFT_TYPES.find(s=>s.id===t.id))];
      setShiftTypes(ordered);
    });
  ...
  const approveConstraintRequest = async (req) => {
    ...
    // Exceptional requests auto-place a shift: École/Examen = 7h day, Repos (or other) = rest
    if (req.type === 'exceptional' && req.date) {
      const emp = employees.find(e => e.id === req.employeeId || e.name === req.employeeName);
      if (emp && emp.storeId) {
        const d = new Date(req.date + 'T12:00');
        const wk = getISOWeek(d), yr = getISOYear(d);
        const dayIdx = (d.getDay() + 6) % 7; // Mon=0..Sun=6
        const excType = req.excType || req.autoType;
        let shift;
        if (excType === 'school' || excType === 'exam') {
          shift = { type: excType, startTime: null, endTime: null, breakH: 0, hours: 7, note: excType === 'exam' ? 'Examen (7h)' : 'École (7h)', depannage: false };
        } else {
          shift = { type: 'rest', startTime: null, endTime: null, breakH: 0, hours: null, note: req.note ? req.note.slice(0, 40) : 'Repos', depannage: false };
        }
        try { await setShift(emp.storeId, wk, yr, emp.id, dayIdx, shift); } catch (e) { /* ignore placement error */ }
      }
    }
  };
  ...
  const getEmpOvertimeToPay = (empId, year, month) => {
    let total = 0;
    // Un mois est "révolu" dès qu'on a dépassé son dernier jour
    const now = new Date();
    const monthIsOver = (year < now.getFullYear()) || (year === now.getFullYear() && month < now.getMonth());
    Object.values(overtimeRecords).forEach(r => {
      ...
      } else if (monthIsOver && r.status !== 'recovered' && r.lastAction !== 'recover') {
        // Fin de mois : les heures supp non traitées basculent automatiquement en "à payer"
        total += (r.extraHours || 0);
      }
    });
    return parseFloat(total.toFixed(2));
  };
