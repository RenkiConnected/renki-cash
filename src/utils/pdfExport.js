...
export async function exportToPDF({ store, employees, schedules, weekDates, shiftTypes, currentWeek, currentYear, logoDataUrl, allStores, getContractH }) {
  ...
  const rows = employees.map(emp => {
    ...
    const cells = weekDates.map((wd, di) => {
      ...
      if (sh.type==='holiday'||sh.type==='exam') totalH += (Number(sh.hours)||7);
...
// ── ALL-STORES PLANNING PDF (dirigeants) — one store per page ──
export async function exportAllStoresPlanningPDF({ stores, employees, schedules, weekDates, shiftTypes, currentWeek, currentYear, logoDataUrl, getContractH }) {
  const stMap = {}; shiftTypes.forEach(st => { stMap[st.id] = st; });
  const workTypes = ['work','communication','meeting','school'];
  const startDate = weekDates[0].date.toLocaleDateString('fr-FR',{day:'numeric',month:'long',year:'numeric'});
  const endDate   = weekDates[6].date.toLocaleDateString('fr-FR',{day:'numeric',month:'long',year:'numeric'});

  function cellHTML(cell) {
    if (cell.empty) return `<td class="sc${cell.isSun?' sun':''}"><div class="empty">—</div></td>`;
    const { sh, st, st2, isSun } = cell;
    return `<td class="sc${isSun?' sun':''}">
      <div class="pill" style="background:${st.bgColor};border-color:${st.color}44">
        ${sh._away && sh._awayStore ? `<span style="display:inline-block;font-size:9px;font-weight:800;color:#fff;background:${sh._awayColor||st.color};border-radius:5px;padding:1px 6px;margin-bottom:2px;">✈ ${sh._awayStore}</span>` : ''}
  ...
  function storePageHTML(store, idx) {
    const storeColor = store.color || '#00C9B1';
    const sched = buildMergedSched(store, employees, schedules, currentYear, currentWeek, stores);
    // Employees of this store + visitors displaced INTO this store
    const homeEmps = employees.filter(e => e.storeId === store.id);
    const visitorIds = new Set();
    Object.keys(sched).forEach(k => {
      const empId = k.split('_')[0];
      if (!homeEmps.find(e => e.id === empId)) visitorIds.add(empId);
    });
    const visitors = employees.filter(e => visitorIds.has(e.id) && e.storeId !== store.id);
    const allEmps = [...homeEmps, ...visitors];
    ...
    return `<div class="page" style="page-break-before:${idx===0?'auto':'always'};">
      <div class="hdr">
        <div class="hdr-left">
          ${logoDataUrl ? `<div class="logo-box"><img src="${logoDataUrl}" alt=""/></div>` : ''}
          <div>
            <div class="store-name" style="color:${storeColor}">${store.name}</div>
            <div class="store-sub">Planning des horaires · Semaine ${currentWeek} · ${currentYear}</div>
          </div>
        </div>
        <div class="week-box" style="background:${storeColor}">
          <div class="week-num">S${currentWeek}</div>
          <div class="week-dates">${startDate}<br/>→ ${endDate}</div>
        </div>
      </div>
      <div class="tbl-wrap">
        <table>
          <thead>
            <tr>
              <th class="emp-th">Employé</th>
              ${weekDates.map(wd => `<th class="${wd.date.getDay()===0?'sun-th':''}"><div class="dn">${wd.day.slice(0,3)}</div><div class="dd">${wd.date.toLocaleDateString('fr-FR',{day:'numeric',month:'short'})}</div></th>`).join('')}
              <th class="tot-th">Total</th>
            </tr>
          </thead>
          <tbody>
            ${rows.length===0 ? `<tr><td colspan="9" style="padding:24px;text-align:center;color:#8A9AA8;">Aucun employé planifié cette semaine.</td></tr>` :
            rows.map(({emp,cells,totalH,diff,isVisitor,homeStore}) => `
              <tr${isVisitor?' style="background:#FFFDF0;"':''}>
                <td class="ec">
                  <span class="av" style="background:${emp.color||storeColor}">${emp.name[0]}</span>
      ...
      <div class="leg">
        ${shiftTypes.map(st => `<div class="li"><div class="ld" style="background:${st.color}"></div><span style="color:${st.color}">${st.label}</span></div>`).join('')}
      </div>
      <div class="ftr">
        <span>Care Planning · ${store.name}</span>
        <span>Généré le ${new Date().toLocaleDateString('fr-FR',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</span>
  ...
  const pagesHTML = stores.map((s, i) => storePageHTML(s, i)).join('');

  const html = `<!DOCTYPE html>
<html lang="fr"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Planning toutes boutiques S${currentWeek}</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
html,body{width:100%;font-family:'Segoe UI',Arial,sans-serif;background:#DDE3E8;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
body{display:flex;flex-direction:column;align-items:center;padding:20px;gap:14px;}
.toolbar{display:flex;gap:10px;width:100%;max-width:1200px;justify-content:flex-end;}
.btn{padding:10px 18px;border:none;border-radius:9px;font-weight:700;font-size:14px;cursor:pointer;color:#fff;}
.b-print{background:#5B6B7A}.b-pdf{background:#00A896}
.toolbar-hint{width:100%;max-width:1200px;font-size:13px;color:#5A6B78;text-align:right;}
.page{width:100%;max-width:1200px;background:#fff;border-radius:14px;box-shadow:0 8px 30px rgba(0,0,0,.12);padding:26px 28px;margin-bottom:16px;}
.hdr{display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;padding-bottom:16px;border-bottom:2px solid #EEF2F5;}
.hdr-left{display:flex;align-items:center;gap:14px;}
.logo-box{height:44px;}.logo-box img{height:44px;object-fit:contain;}
.store-name{font-size:26px;font-weight:800;}
.store-sub{font-size:13px;color:#5A6B78;margin-top:2px;}
.week-box{color:#fff;border-radius:12px;padding:8px 16px;text-align:center;}
.week-num{font-size:20px;font-weight:800;}
.week-dates{font-size:10px;opacity:.92;line-height:1.3;margin-top:2px;}
.tbl-wrap{overflow-x:auto;}
table{width:100%;border-collapse:separate;border-spacing:3px;}
th{padding:7px 4px;font-size:11px;font-weight:700;color:#5A6B78;text-align:center;}
.emp-th{text-align:left;padding-left:8px;min-width:150px;}
.sun-th{color:#C8002B;}
.tot-th{color:#00A896;}
td{vertical-align:middle;}
.ec{display:flex;align-items:center;gap:8px;padding:6px 8px;}
.av{width:28px;height:28px;border-radius:50%;color:#fff;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;flex-shrink:0;}
.en{font-size:13px;font-weight:700;display:flex;flex-direction:column;}
.es{font-size:10px;font-weight:500;color:#8A9AA8;}
.sc{padding:0;}.sc.sun{opacity:.6;}
.pill{border:1.5px solid;border-radius:9px;padding:5px 3px;display:flex;flex-direction:column;align-items:center;gap:1px;min-height:38px;justify-content:center;}
.pill .lbl{font-size:10px;font-weight:700;}
.pill .tm{font-size:9.5px;font-weight:600;}
.pill .hr{font-size:10px;font-weight:800;}
.empty{color:#C8D2DA;font-size:14px;text-align:center;}
.sep{width:70%;height:1px;margin:2px 0;opacity:.3;}
.tot{text-align:center;}
.tot-h{font-size:14px;font-weight:800;}
.tot-d{font-size:10px;font-weight:700;}
.c-ok{color:#1A8A42;}.c-over{color:#B05A00;}.c-under{color:#C8002B;}
.leg{display:flex;flex-wrap:wrap;gap:12px;margin-top:16px;padding-top:14px;border-top:1px solid #EEF2F5;}
.li{display:flex;align-items:center;gap:5px;font-size:11px;font-weight:600;}
.ld{width:12px;height:12px;border-radius:3px;}
.ftr{display:flex;justify-content:space-between;margin-top:14px;font-size:11px;color:#8A9AA8;}
@media print{
  body{display:block;background:#fff;padding:0;margin:0;}
  .toolbar,.toolbar-hint,.no-print{display:none!important;}
  .page{width:100%;max-width:none;border-radius:0;box-shadow:none;margin:0;padding:8mm;}
  @page{size:A4 landscape;margin:5mm;}
}
</style></head>
<body>
...
<div class="toolbar-hint no-print">Toutes les boutiques · une par page. Pour le PDF : dans la fenêtre d'impression, choisissez « Enregistrer au format PDF ».</div>
${pagesHTML}
</body></html>`;
...
function _empLeaveDays(leaveRequests, empId, year, month) {
  const days = new Set();
  (leaveRequests || []).filter(r => r.employeeId === empId && r.status === 'approved').forEach(req => {
    (req.weeks || []).forEach(w => {
      const wd = _pmWeekDates(w.week, w.year);
      (w.days || []).forEach(di => { const date = wd[di]; if (date && date.getMonth() === month && date.getFullYear() === year) days.add(date.getDate()); });
    });
  });
  return [...days].sort((a, b) => a - b);
}
function _empLateness(schedules, empId, year, month) {
  const entries = []; let total = 0;
  Object.entries(schedules || {}).forEach(([key, sched]) => {
    const m = key.match(/_(\d{4})_W(\d+)$/); if (!m) return;
    const kYear = parseInt(m[1]), wn = parseInt(m[2]);
    const wd = _pmWeekDates(wn, kYear);
    for (let di = 0; di < 7; di++) {
      const sh = sched[`${empId}_${di}`];
      if (!sh || !(sh.latenessMin > 0)) continue;
      const date = wd[di];
      if (date && date.getMonth() === month && date.getFullYear() === year) { entries.push({ date: date.getDate(), min: sh.latenessMin }); total += sh.latenessMin; }
    }
  });
  entries.sort((a,b)=>a.date-b.date);
  return { total, entries };
}
function _buildStorePrime(store, employees, sd, getOvertime, year, month, leaveRequests, schedules) {
  ...
  const rows = emps.map(emp=>{
    ...
    const autoOt = (getOvertime && year!=null && month!=null && !v.ignoreAutoOvertime) ? getOvertime(emp.id, year, month) : 0;
    ...
    return { name:emp.name, role:emp.role, base, share, total:base+share, travel:Number(v.travel)||0, overtime, detail, sharePct:Number(v.storeBonusPct)||0,
      hasLateness:!!v.hasLateness, latenessNote:v.latenessNote||'', hasAbsence:!!v.hasAbsence, absenceNote:v.absenceNote||'',
      leaveDays:_empLeaveDays(leaveRequests, emp.id, year, month), lateness:_empLateness(schedules, emp.id, year, month) };
  });
    ...
    <div style="padding:6px 0;background:#fff;">
      ${data.rows.length===0 ? `<div style="padding:16px 22px;color:#9EBBCA;font-style:italic;">Aucun collaborateur</div>` : data.rows.map(r=>`
        <div style="padding:12px 22px;border-bottom:1px solid #EEF3F5;">
          ...
          ${(r.hasLateness||r.hasAbsence||r.leaveDays.length>0||(r.lateness&&r.lateness.total>0))?`<div style="margin-top:6px;display:flex;gap:8px;flex-wrap:wrap;">
            ${(r.lateness&&r.lateness.total>0)?`<span style="background:#FFF0F2;color:#C8002B;border:1px solid #F5B5C0;border-radius:7px;padding:3px 10px;font-size:12px;font-weight:700;">⏰ Retards : ${r.lateness.total} min (${r.lateness.entries.map(e=>`${e.date}: ${e.min}min`).join(', ')})</span>`:''}
            ${r.hasLateness?`<span style="background:#FFF0F2;color:#C8002B;border:1px solid #F5B5C0;border-radius:7px;padding:3px 10px;font-size:12px;font-weight:700;">⏰ Retards${r.latenessNote?` : ${r.latenessNote}`:''}</span>`:''}
            ${r.hasAbsence?`<span style="background:#FFF7E0;color:#B05A00;border:1px solid #F5C96B;border-radius:7px;padding:3px 10px;font-size:12px;font-weight:700;">🚫 Absences${r.absenceNote?` : ${r.absenceNote}`:''}</span>`:''}
            ${r.leaveDays.length>0?`<span style="background:#F3F0FF;color:#7C3AED;border:1px solid #C4B5FD;border-radius:7px;padding:3px 10px;font-size:12px;font-weight:700;">🌴 Congés : ${r.leaveDays.length} j (${r.leaveDays.join(', ')})</span>`:''}
          </div>`:''}
        </div>`).join('')}
...
export async function exportPrimesPDF({ storesData, month, year, scope, mode }) {
  ...
  try {
    ...
    document.body.removeChild(node); // we render store-by-store instead

    const renderNode = async (html) => {
      const n = document.createElement('div');
      n.style.cssText = `position:fixed;left:-99999px;top:0;width:900px;background:#fff;font-family:'Segoe UI',Arial,sans-serif;padding:32px;color:#1B2A3B;`;
      n.innerHTML = html;
      document.body.appendChild(n);
      const c = await html2canvas(n, { scale: 2, backgroundColor:'#ffffff', logging:false, useCORS:true });
    ...
    // One PDF page per store (no cut between stores)
    for (let i = 0; i < storesData.length; i++) {
      const sd = storesData[i];
      const html = (i === 0 ? headerHTML : '') + _storeTableHTML(sd.store, sd.data);
      const canvas = await renderNode(html);
      ...
      if (imgH <= ph - margin*2) {
        ...
        // Single store taller than a page: split only this store
    ...
    if (mode === 'preview') {
      ...
      if (!win) pdf.save(`${fname}-${MONTHS_FR[month]}-${year}.pdf`);
    } else {
...
// Vacation overview as a JPG/print-friendly image for dirigeants
export async function exportVacationsVisuel({ rows, stores, month, year, daysInMonth }) {
  const dayList = Array.from({ length: daysInMonth }, (_, i) => i + 1);
  const storeOf = emp => (stores || []).find(s => s.id === emp.storeId);
  const node = document.createElement('div');
  node.style.cssText = `position:fixed;left:-99999px;top:0;background:#fff;font-family:'Segoe UI',Arial,sans-serif;padding:32px;`;
  const cell = 26;
  node.innerHTML = `
    <div style="min-width:${180 + daysInMonth * cell + 40}px;">
      <div style="font-size:30px;font-weight:800;color:#1B2A3B;margin-bottom:4px;">🏖️ Vacances — ${MONTHS_FR[month]} ${year}</div>
      <div style="font-size:15px;color:#5A6B78;margin-bottom:20px;">${rows.length} personne(s) en vacances ce mois</div>
      <div style="display:grid;grid-template-columns:180px repeat(${daysInMonth}, ${cell}px);gap:2px;align-items:center;margin-bottom:8px;">
        <div style="font-size:12px;font-weight:800;color:#5A6B78;">Employé</div>
        ${dayList.map(d => { const dow = new Date(year, month, d).getDay(); const we = dow === 0 || dow === 6; return `<div style="font-size:10px;text-align:center;font-weight:700;color:${we ? '#C8002B' : '#5A6B78'};">${d}</div>`; }).join('')}
      </div>
      ${rows.map(({ emp, days }) => {
        const st = storeOf(emp); const daySet = new Set(days);
        return `<div style="display:grid;grid-template-columns:180px repeat(${daysInMonth}, ${cell}px);gap:2px;align-items:center;margin-bottom:4px;">
          <div style="display:flex;align-items:center;gap:7px;">
            <div style="width:24px;height:24px;border-radius:50%;background:${emp.color || '#00C9B1'};color:#fff;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;">${emp.name[0]}</div>
            <div><div style="font-size:13px;font-weight:700;">${emp.name}</div><div style="font-size:10px;color:#5A6B78;">${st ? st.name : '—'}</div></div>
          </div>
          ${dayList.map(d => { const on = daySet.has(d); const dow = new Date(year, month, d).getDay(); const we = dow === 0 || dow === 6; return `<div style="height:24px;border-radius:5px;background:${on ? '#7C3AED' : (we ? '#F4F4F6' : '#F6FAFB')};"></div>`; }).join('')}
        </div>`;
      }).join('')}
      <div style="margin-top:16px;font-size:13px;color:#5A6B78;">Généré par Care Planning · ${new Date().toLocaleDateString('fr-FR')}</div>
    </div>`;
  document.body.appendChild(node);
  try {
    const canvas = await html2canvas(node, { scale: 2, backgroundColor: '#ffffff', logging: false, useCORS: true });
    ...
    canvas.toBlob((blob) => {
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = `vacances-${MONTHS_FR[month]}-${year}.jpg`.toLowerCase();
      a.click();
      setTimeout(() => URL.revokeObjectURL(url), 5000);
    }, 'image/jpeg', 0.95);
  } catch (err) {
